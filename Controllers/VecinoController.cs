    ...
    public class VecinoController : Controller
    {
                ...
                using (M_VPSA_V3Context bd = new M_VPSA_V3Context())
                {
                    using (var transaccion = new TransactionScope())
                    {
                        if (oVecinoCLS.IdVecino == 0)
                        {
                            ...
                            oPersona = bd.Persona.Where(p => p.Dni == oVecinoCLS.Dni).First();
                        ...
                        {    //FaltaTerminar LA EDICION
                            Usuario oUsuario = bd.Usuario.Where(p => p.IdUsuario == oVecinoCLS.IdVecino).First();
                            oUsuario.NombreUser = oVecinoCLS.NombreUser;
        ...
        public int validarCorreo(int id, string correo)
        {
            ...
            try
            {
                using (M_VPSA_V3Context bd = new M_VPSA_V3Context())
                {
                    ...
                    {
                        rpta = bd.Persona.Where(p => p.Mail.ToLower() == correo.ToLower()).Count();
                    }
                    else
                    {
                        rpta = bd.Persona.Where(p => p.Mail.ToLower() == correo.ToLower() && p.IdPersona != id).Count();
                    }
                }
            }
            catch (Exception ex)
        ...
        public VecinoCLS login([FromBody] VecinoCLS oVecinoCLS)
        {
            ...
            using (M_VPSA_V3Context bd = new M_VPSA_V3Context())
            {
                ...
                rpta = bd.UsuarioVecino.Where(p => p.NombreUser.ToUpper() == oVecinoCLS.NombreUser.ToUpper() && p.Contrasenia == claveCifrada).Count();
                if (rpta == 1)
                {
                    UsuarioVecino oUsuarioRecuperar = bd.UsuarioVecino.Where(p => p.NombreUser.ToUpper() == oVecinoCLS.NombreUser.ToUpper()
