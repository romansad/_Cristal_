...
namespace MUNICIPALIDAD_V4.Controllers
{
    public class UsuarioController : Controller
    {
        ...
        public UsuarioCLS RecuperarUsuario(int idUsuario)
        {
            using (M_VPSA_V3Context bd = new M_VPSA_V3Context())
            {
                UsuarioCLS oUserCLS = (from Usuario in bd.Usuario
                                       join Persona in bd.Persona
                                       on Usuario.IdPersona equals Persona.IdPersona
                                       join Rol in bd.Rol on Usuario.IdTipoUsuario equals Rol.IdRol
                                       where Usuario.Bhabilitado == 1
            ...
            try
            {
                using (M_VPSA_V3Context bd = new M_VPSA_V3Context())
                {
                    if (oUsuarioCLS.IdUsuario == 0)
                    {
                        ...
                        oPersona = bd.Persona.Where(p => p.Dni == oUsuarioCLS.Dni).First();
        ...
        public List<PaginaCLS> ListarPaginas()
        {
            ...
            using (M_VPSA_V3Context bd = new M_VPSA_V3Context())
            {
                listaPagina = (from paginaRol in bd.Paginaxrol
        ...
        public SeguridadCLS obtenerVariableSession()
        {
            ...
            else
            {
                ...
                using (M_VPSA_V3Context bd = new M_VPSA_V3Context())
                {
                    listaPaginaCLS = (from empleado in bd.Usuario
                                      join rol in bd.Rol
          on empleado.IdTipoUsuario equals rol.IdRol
                                      join paginaxrol in bd.Paginaxrol
        ...
        public UsuarioCLS login([FromBody] UsuarioCLS oUsuarioCLS)
        {
            ...
            using (M_VPSA_V3Context bd = new M_VPSA_V3Context())
            {
                ...
                if (rpta == 1)
                {
                    Usuario oUsuarioRecuerar = bd.Usuario.Where(p => p.NombreUser.ToUpper() == oUsuarioCLS.NombreUser.ToUpper()
